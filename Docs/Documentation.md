...
- **Sleeve broker orbital trader** (`altered.sleevemarket`, richness slider 1–8, default 3) — Adds an occasional orbital trader that gives the transhuman trade a buy-side: empty sleeve bodies grown ready to cast into, empty neural stacks, a nano storage drive to bootstrap the research if you haven't unlocked it yet, and, with Biotech, genepacks. Everything is priced by its own high market value. The richness slider caps how many sleeves, stacks and genepacks it carries per visit.
- **"Altered Carbon: Digitized" scenario** (`altered.scenario`) — A solo, naked-brutality-tier start. Your colonist wakes as an empty sleeve, their digitized mind held in a derelict neural matrix inside a sealed, steel-walled bunker room that is built at the map start, containing the matrix and a malfunctioning casting relay tuned to it. The matrix runs on its own sealed power cell (it needs no grid and consumes no power), the mind actively needlecasts into the sleeve through an installed neural receiver, and the malfunctioning relay pins your casting range to a single tile. Guard the matrix: if it is destroyed, the connection drops and the sleeve goes empty. You start naked with nothing but the bunker and the sleeve you wear - no food, medicine, or supplies.
...
### VPE - Biotech Integration

With VPE - Biotech Integration, access to a psycast path is a genetic trait: a pawn needs the matching psycast gene to take the path. Out of the box that covers Vanilla Psycasts Expanded's own paths and a handful of vanilla and VRE xenotypes. These features extend the same idea to the whole modlist. Both need Biotech, Vanilla Psycasts Expanded and VPE - Biotech Integration.

- **`vpebiotech.pathgenes`** — every psycast path that anyone could freely take gains its own gating gene, in the same style as the existing psycast genes: the Runesmith path, the Sentinel paths (Aeromancer, Animancer, Biohazard, Biosoother, Bugmancer, Deadlife, Fleshshaper, Awoken Gauranlen, Geomancer, Gravcaster, Hydromancer), the four Alpha Animals paths (Herald of the Black Hive, Oculist, Ravager, Xeno-seer) and Combat Psycasts' Kinecticist — seventeen genes, each with original art. Every new gene also joins the psycast gene node when the Genetics Research Overhaul's gene nodes are enabled, so a colony can still grow a caster of any school. Paths already guarded another way — mechanitor-only paths, void-focus paths — keep their own gate and get no gene.
- **`vpebiotech.xenotypes`** — every xenotype in the modlist that had no psycast gene receives one to three thematically fitting ones, xenotypes VPE - Biotech Integration does not cover get full thematic rosters (the Alpha Genes and WVC families, Rimsenal, lycans and more), and already-covered xenotypes get natural additions: dirtmoles gain the Geomancer gene, wasters Biohazard and the Herald gene, starjacks Gravcaster, hussars Ravager and Kinecticist, the phytokin family the genes of their own trees. Deepkin dig with Geomancer and Runesmith, boglegs channel their swamps, vulpids get foxfire, reanimates raise deadlife, and so on across roughly a hundred xenotypes. Each assignment only applies when the mods owning the gene and the xenotype are both loaded.
...
With VPE - Biotech Integration, each psycast path is locked behind a xenotype gene that otherwise only arrives on a pawn born with it. A **psycast gene nodes** project sits at the far end of the tree past archite gene nodes, and unlocks an archite gene node carrying every one of those path genes, so a colony can grow psycasters of any school instead of waiting for the right pawn to show up. It is priced as an endgame build: archite capsules, spacer components, silver and eltex. The node also carries every gating gene the VPE - Biotech Integration features add for other mods' paths, so adding a new gated path later extends the node on its own.
...
- **Cyberbrains for psycasters** (`cybernetics.echobrains`) — Adds two Echo psycaster cyberbrains and cuts the meditation bonus other cyberbrains gave as a side effect, so psychic capability is a choice rather than a freebie on a combat brain. With Vanilla Psycasts Expanded, the two Echo brains also cost eltex to craft (7 for the SEER, 15 for the ORACLE), on top of their inherited materials. Needs GiTS Cyberbrains.
