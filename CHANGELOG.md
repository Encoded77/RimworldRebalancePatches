...
## 1.18.0 (2026-07-31)
- **VPE - Biotech Integration** - new settings group. Needs Biotech, Vanilla Psycasts Expanded and VPE - Biotech Integration.
  - **Every free psycast path gets a gating gene** (`vpebiotech.pathgenes`) - seventeen paths were open to any psycaster while Vanilla Psycasts Expanded's own paths needed a gene: Runesmith, the eleven Sentinel paths (Aeromancer, Animancer, Biohazard, Biosoother, Bugmancer, Deadlife, Fleshshaper, Awoken Gauranlen, Geomancer, Gravcaster, Hydromancer), the four Alpha Animals paths (Herald of the Black Hive, Oculist, Ravager, Xeno-seer) and Combat Psycasts' Kinecticist. Each now has its own psycast gene in the established style, with original icons, and each joins the psycast gene node when the Genetics Research Overhaul's gene nodes are on. Mechanitor-only and void-focus paths keep their own gates and get no gene.
  - **Psycast genes across all xenotypes** (`vpebiotech.xenotypes`) - roughly a hundred xenotypes get thematic psycast genes: every xenotype without one receives one to three (deepkin: Geomancer and Runesmith; boglegs: Hydromancer and Biohazard; vulpids: mind, foxfire and puppetry; reanimates: Deadlife and Necropath; and so on across Alpha Genes, Big and Small, Det's, Rimsenal, WVC and more), families VPE - Biotech Integration does not reach get full rosters, and covered xenotypes gain natural additions: dirtmoles the Geomancer gene, wasters Biohazard and the Herald gene, hussars Ravager and Kinecticist, starjacks Gravcaster, the phytokin family the genes of their own trees.
  - Heads-up: Psycasts² ships its "disable gene requirements" setting on by default, which silently strips the gene gate from every psycast path at startup - VPE - Biotech Integration's included. Turn it off for any of this gating to matter.

## 1.17.0 (2026-07-30)
- **Psycast Overhaul** - new overhaul group, off by default. Needs Vanilla Psycasts Expanded.
  - **Psychic research tab and gear chain** (`psycast.research`) - a Psychic tab with three chained projects: psychic attunement (Medieval, takes over VPE's caster gear: eltex mask, cape and eltex melee weapons), eltex working (Spacer, takes over VPE's eltex gear project and gates all eltex apparel crafting; two Empire techprints, also reachable through the deserter network), and psychic armorweave (Ultra). The two absorbed VPE projects are removed so nothing is researchable twice.
  - **Eltex Weaponry joins the psychic tab** (`psycast.eltexweaponry`) - its research moves onto the tab, requires eltex working plus Fabrication, and keeps its own two Empire techprints.
  - **Prestige armor needs psychic armorweave** (`psycast.prestigegate`) - every craftable prestige combat armor that grants psychic sensitivity or is built from eltex, from any mod, additionally requires the armorweave project; the base armor's own research still applies. Armor that only carries the prestige tag so nobles will wear it, with no psychic thread, is left alone.
- The Civis flagship cyberbrain research is now `RBP_CybCivisFlagship` (was `RBP_CybCivisPX7`): the game refuses generated techprint items whose name ends in a digit, and the old name produced exactly that. Saves migrate automatically - research progress and any techprint items carry over, nothing to do.
- Fixed the Echo SEER and ORACLE cyberbrain hediffs carrying two comps lists, which duplicated any comp later patched onto them; the eltex diadem's hair-visibility used two apparel fields that no longer exist (now `renderSkipFlags`); the database dump no longer triggers the game's "failed to find sane damage" error on projectiles that declare no damage.
- The test harness now fails a run when the log carries config errors naming this mod's own defs, so a green suite can no longer ship alongside rejected XML.
- Settings menu: the Psycast Overhaul now sits with the other overhauls above the divider, and Pawn augmentation moves below it - it is a world-generation tweak, not an overhaul, and no longer claims the overhaul styling.
- **Biological warfare research is now two projects, not fifteen** (`biowarfare.researchtree`) - the previous staged tree asked for roughly 19,000 research points across eighteen nodes, which made specialising into biological warfare cost far more than the content behind it. **Pathogen weaponisation** (1200) now unlocks every disease's barrels, shells, grenades, missiles and vents at once, and **broad-spectrum antibodies** (1500) unlocks every vaccine and antiserum, still studied at the antigens analyzer with a sample loaded but accepting any sample. The per-disease projects and the staging nodes are retired, and everything that pointed at them is repointed. Total cost drops to 3,500.
- **Vali's Charged Arsenal - Eltex Series** - new settings group.
  - **Charge eltex weapons carried by psycasters** (`valicharged.psyspawns`) - the nine charge eltex melee weapons are reward and trade items whose spawn tag nothing uses, so no pawn ever carries one. They gain the psychic-melee tag and join the pool Empire psycasters and deserters draw from when Eltex Weaponry's psychic-gear spawning is active.
  - **Psychic implants need eltex working** (`psycast.echotie`) - the Cybernetics Research Overhaul's psychic hardware lane opens from eltex working when both overhauls are on, since every psychic implant is built from eltex.
  - **Eltex robe and skullcap reward dedication** (`psycast.eltexbuff`) - the two slots where a psycaster gives up real armor now pay for the choice. Robe: +30% psychic sensitivity, +0.1 neural heat recovery, +20% meditation psyfocus, 10% cheaper psycasts. Skullcap: +50% sensitivity, +0.1 recovery, +10% meditation, 5% cheaper casts. Prestige armor is untouched: an armored caster keeps everything, a robed one is simply better at the craft.
  - **Eltex trousers** (`psycast.trousers`) - the missing legs piece of the eltex wardrobe: +10% psychic sensitivity, faster heat recovery, fits under any armor. Crafted behind eltex working. Original art.
  - **Psychic vestments** (`psycast.vestments`) - a new project past eltex working unlocks the eltex vestment and eltex diadem, endgame caster garments with heavy operating stats and almost no armor, rare quest rewards, never sold, each refusing wearers below a psychic sensitivity threshold (175% / 150%, checked when dressing). Original art.
  - **The Echo ORACLE opens one locked path** (`psycast.oraclewaiver`) - a pawn carrying the Echo ORACLE cyberbrain may unlock one psycast path they are otherwise locked out of, whatever gene, backstory or belief guards it. The brain remembers its one opened path; the path survives brain removal, and a replacement ORACLE carries a fresh waiver. Needs GiTS Cyberbrains with Echo cyberbrains.
- **Psycaster gear report** - the arsenal analyzer now emits `psygear.md`: every apparel and weapon granting a psychic stat, the prestige family, quality-scaled vs flat sensitivity kept apart, and a best-per-slot table comparing the robed and armored loadouts.
- **Better Quest Rewards** - new settings group.
  - **No quest rewards that cannot exist** (`questrewards.psytrainers`) - Better Quest Rewards offers Vanilla Psycasts Expanded psytrainers as quest rewards, while Fuck Psytrainers 2 removes every psytrainer from the game. With all three loaded that one reward option names 148 items that never exist, and it logs an unresolved cross-reference for each one on every load. The dead option is dropped, the rest of the reward table is untouched.
- **Psycast gene node at the end of the tree** (`geneticsresearch.psycastnodes`) - VPE - Biotech Integration locks each psycast path behind a xenotype gene, which otherwise only arrives on a pawn born with it. Adds a **psycast gene nodes** project past archite gene nodes on the genetics tab, and the archite gene node it unlocks, carrying every path gene so a colony can grow psycasters of any school. Priced as an endgame build: archite capsules, spacer components, silver and eltex. The node also picks up the Alpha Animals path genes from that add-on, and More Psycaster Genes' path genes for whichever psycast mods it supports and you run, so a gated path added later joins the node without another patch. Part of the Genetics Research Overhaul.
- **Vanilla Psycasts Expanded** - new settings group.
  - **Pawn generation survives locked psycast paths** (`vpe.pawngenguard`) - Vanilla Psycasts Expanded picks a random path when it promotes a generated pawn to psycaster, and mods that gate paths behind genes can leave an ordinary pawn with nothing to pick, at which point the pick fails and takes the pawn's whole generation with it. Pawns who could not have become a psycaster anyway now skip that roll, so raids, quests and caravans arrive instead of erroring.
- Vanilla Psycasts Expanded - Anima's Anima-Link Neuroformer is recorded as outside the archotech shard recycling loop, alongside the vanilla psylink neuroformer and the Royalty psychic implants, so psylink stays something you find rather than something you assemble.

## 1.16.0 (2026-07-30)
- **Vacuum resistance trims, whole modlist** (`odyssey.vacuumtrims`) — the feature now covers every piece of modded armor that broke Vanilla Gravship Expanded's vacuum ladder, instead of the handful it started with. That mod's rule for vanilla gear is that a helmet is worth at most 65% and a body 32%, so no single item is enough; modded armor is now held to it. Rimsenal - Harana, Alpha Genes, Eventide Warcasket, Spacer Pack, the Alpha Mechs mech controller warcasket and Rimsenal's hazard carapace set join the existing trims, and the Impact Weaponry crusader helmet drops from 0.67 to 0.65 to sit under the same cap. Two items were far outside it: Altered Carbon's enviro suit helmet granted 97% on its own, and the Spacer Pack pulsepack granted 70% from a belt, which covers nothing and stacks with any armor.
